...
<div class="max-w-4xl mx-auto p-6 bg-white rounded-lg shadow">
    <h2 class="text-2xl font-bold mb-6">Modifier la réservation #{{ $reservation->id }}</h2>
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
            ...
            <div class="col-span-1">
                ...
                <input type="number" name="total" step="0.01" min="0" required
                    value="{{ old('total', $reservation->total) }}"
                    class="w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500">
                @error('total')
                    <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                @enderror
            ...
            <div class="col-span-1">
                ...
                <select name="status" required
                    class="w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500">
                    <option value="Confirmé" {{ old('status', $reservation->status) == 'Confirmé' ? 'selected' : '' }}>Confirmé</option>
                    <option value="En attente" {{ old('status', $reservation->status) == 'En attente' ? 'selected' : '' }}>En attente</option>
                    <option value="Payé partiellement" {{ old('status', $reservation->status) == 'Payé partiellement' ? 'selected' : '' }}>Payé partiellement</option>
                    <option value="Annulé" {{ old('status', $reservation->status) == 'Annulé' ? 'selected' : '' }}>Annulé</option>
                </select>
                @error('status')
                    <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                @enderror
